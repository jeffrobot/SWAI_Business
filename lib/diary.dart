...
class _DiaryPageState extends State<DiaryPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('내 Mind Diary',style: TextStyle(color: Color(0xffF3E7FF)),),
            ...
            Container(
                ...
                child: TextButton(
                  ...
                  onPressed: (){
                    ...
                    gemini.text(
                      """
                    '${_textEditingController.text}'. From this diary, give me a single line summary of how he or she feels based on the event. Start with 'Event: ..., Emotion: ...'.
                    """
                    ).then((value){
                      print("This is the result:");
                      // print(value?.content?.parts?.last.text);
                      String content = value?.content?.parts?.last.text.toString() ?? '';
                      print(content);
                      // crud.addOrUpdateEmotion(email, value?.content?.parts?.last.text.toString() ?? '');
                      crud.createEmo(email, content).then((value){
                      _textEditingController.clear();
                    });
                    });
                  
                  },
                  child: Text('일기 저장', style: TextStyle(color: Colors.white),)
                ),
              ),
          ],
        ),
